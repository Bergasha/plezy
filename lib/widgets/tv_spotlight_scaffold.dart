...
class _TvSpotlightThemeMusicListenerState extends State<_TvSpotlightThemeMusicListener>
  ...
  /// Cached in [didChangeDependencies]: by the time [dispose] runs the
  /// element may already be deactivated, and `context.read` on a deactivated
  /// element throws ("Looking up a deactivated widget's ancestor is
  /// unsafe") rather than returning null like every other call site here.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    unawaited(
      state == AppLifecycleState.resumed ? service?.resume(_themeMusicOwner) : service?.pause(_themeMusicOwner),
    );
  }
